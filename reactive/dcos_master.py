...
def install_dcosmaster():
    ...
    open(configdir+'roles/slave', 'a').close()
    ...
    text_file.writelines(["EXHIBITOR_BACKEND=STATIC","EXHIBITOR_STATICENSEMBLE=1:172.31.10.191"])
    text_file.close()
    text_file=open(basedir+"packages/dcos-config--setup_b3e41695178e35239659186b92f25820c610f961/etc/master_list", 'w')
    text_file.write("[\"172.31.10.191\"]")
    text_file.close()
    log("remove dnsmasq")
    dnsprocess = check_output(["apt-get", "remove", '-y', "dnsmasq-base"])
    log("install")
